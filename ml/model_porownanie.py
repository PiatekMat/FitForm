...
logging.info("--- WYNIKI XGBOOST ---")
logging.info(f"Błędy RMSE w poszczególnych próbach (kg): {rmse_scores_xgb}")
logging.info(f"Średni błąd modelu (RMSE): {rmse_scores_xgb.mean():.4f} kg")

error_percentage_xgb = (rmse_scores_xgb.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd predykcji względem wagi: {error_percentage_xgb:.2f}%")
...
logging.info("--- WYNIKI LIGHTGBM ---")
logging.info(f"Błędy RMSE w poszczególnych próbach (kg): {rmse_scores_lgb}")
logging.info(f"Średni błąd modelu (RMSE): {rmse_scores_lgb.mean():.4f} kg")

error_percentage_lgb = (rmse_scores_lgb.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd predykcji względem wagi: {error_percentage_lgb:.2f}%")
...
logging.info("--- WYNIKI CATBOOST ---")
logging.info(f"Błędy RMSE w poszczególnych próbach (kg): {rmse_scores_cat}")
logging.info(f"Średni błąd modelu (RMSE): {rmse_scores_cat.mean():.4f} kg")

error_percentage_cat = (rmse_scores_cat.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd predykcji względem wagi: {error_percentage_cat:.2f}%")
...
logging.info("--- WYNIKI RANDOMFOREST ---")
logging.info(f"Błędy RMSE w poszczególnych próbach (kg): {rmse_scores_rf}")
logging.info(f"Średni błąd modelu (RMSE): {rmse_scores_rf.mean():.4f} kg")

error_percentage_rf = (rmse_scores_rf.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd predykcji względem wagi: {error_percentage_rf:.2f}%")
...
logging.info("--- WYNIKI REGRESJI LINIOWEJ ---")
logging.info(f"Błędy RMSE (kg): {rmse_scores_lr}")
logging.info(f"Średni błąd (RMSE): {rmse_scores_lr.mean():.4f} kg")

error_percentage_lr = (rmse_scores_lr.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd: {error_percentage_lr:.2f}%")
...
logging.info("--- WYNIKI ELASTIC NET ---")
logging.info(f"Błędy RMSE (kg): {rmse_scores_en}")
logging.info(f"Średni błąd (RMSE): {rmse_scores_en.mean():.4f} kg")

error_percentage_en = (rmse_scores_en.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd: {error_percentage_en:.2f}%")
...
logging.info("--- WYNIKI RIDGE REGRESSION ---")
logging.info(f"Błędy RMSE (kg): {rmse_scores_ridge}")
logging.info(f"Średni błąd (RMSE): {rmse_scores_ridge.mean():.4f} kg")

error_percentage_ridge = (rmse_scores_ridge.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd: {error_percentage_ridge:.2f}%")
...
logging.info("--- WYNIKI SVM ---")
logging.info(f"Błędy RMSE (kg): {rmse_scores_svm}")
logging.info(f"Średni błąd (RMSE): {rmse_scores_svm.mean():.4f} kg")

error_percentage_svm = (rmse_scores_svm.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd: {error_percentage_svm:.2f}%")
...
logging.info("--- WYNIKI KNN ---")
logging.info(f"Błędy RMSE (kg): {rmse_scores_knn}")
logging.info(f"Średni błąd (RMSE): {rmse_scores_knn.mean():.4f} kg")

error_percentage_knn = (rmse_scores_knn.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd: {error_percentage_knn:.2f}%")
...
logging.info("--- WYNIKI DECISION TREE ---")
logging.info(f"Błędy RMSE (kg): {rmse_scores_dt}")
logging.info(f"Średni błąd (RMSE): {rmse_scores_dt.mean():.4f} kg")

error_percentage_dt = (rmse_scores_dt.mean() / df_clean['waga_czczo'].mean()) * 100
logging.info(f"Procentowy błąd: {error_percentage_dt:.2f}%")
...
wyniki_xgb = symulacja_wagi(model_xgb, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_lgb = symulacja_wagi(model_lgb, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_cat = symulacja_wagi(model_cat, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_rf = symulacja_wagi(model_rf, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_lr = symulacja_wagi(pipeline_lr, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_en = symulacja_wagi(pipeline_en, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_ridge = symulacja_wagi(pipeline_ridge, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_svm = symulacja_wagi(pipeline_svm, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_knn = symulacja_wagi(pipeline_knn, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
wyniki_dt = symulacja_wagi(model_dt, dni_prognozy, waga_start, kalk_kcal, kalk_bialko, kalk_spalone, kalk_cardio, kalk_silowy, kalk_kroki)
